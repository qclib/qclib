...
class PivotStatePreparation:
    def __init__(self, state, aux=False):
        ...
        if aux:
            self.sp_circuit = qiskit.QuantumCircuit(anc, memory)
            nun_aux = n_anci - 1
            self.sp_circuit.compose(initialize_circ,
                               self.sp_circuit.qubits[nun_aux:nun_aux + target_size],
                               inplace=True)

            self.sp_circuit.barrier()
            self.sp_circuit.compose(pivot_circuit.reverse_bits().reverse_ops(), inplace=True)
        else:
            self.sp_circuit = qiskit.QuantumCircuit(n_qubits)
            self.sp_circuit.compose(initialize_circ, self.sp_circuit.qubits[:target_size], inplace=True)

            self.sp_circuit.compose(pivot_circuit.reverse_bits().reverse_ops(),
