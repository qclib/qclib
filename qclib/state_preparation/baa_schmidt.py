...
def initialize(state_vector, max_fidelity_loss=0.0,
    ...
    for i, (vec, low_rank) in enumerate(zip(node.vectors, node.k_approximation)):
        qc_vec = schmidt.initialize(
            vec, isometry_scheme=isometry_scheme, unitary_scheme=unitary_scheme, low_rank=low_rank
        )
