...
""" Tests for qclib.entanglement.py """
...
class TestEntanglement(TestCase):
    """Tests geometric entanglement"""

    def test_geometric_measure_entanglement(self):
        """test geometric entanglement with ghz states"""
        ghz4 = np.zeros(16)
        ghz4[0] = 1 / np.sqrt(2)
        ghz4[15] = 1 / np.sqrt(2)
        ...
        nqbits = 5
        w6_vector = np.zeros(2**nqbits)
        for k in range(nqbits):
            w6_vector[2**k] = 1 / np.sqrt(nqbits)

        gme = geometric_entanglement(w6_vector)
    ...
    def test_geometric_positive(self):
        ...
        self.assertTrue(geometric_entanglement(vector) > 0)
