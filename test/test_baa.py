...
def get_vector(
    ...
    if isinstance(start_depth_multiplier, int):
        ...
        multiplier = np.random.randint(
            start_depth_multiplier[0], start_depth_multiplier[1]
        )
    else:
        raise ValueError(
            "start_depth_multiplier must be either an int or an array-like of int."
        )
    ...
    while e_lower > entanglement or entanglement > e_upper:
        np.random.seed()
        q_circuit: qiskit.QuantumCircuit = random_circuit(
            num_qubits, multiplier * num_qubits
        )
...
def execute_experiment(
    ...
    state_vector, entganglement, depth = get_vector(
        *entanglement_bounds,
        num_qubits,
        start_depth_multiplier=start_depth_multiplier,
        measure="geometric",
    )
    ...
    (
        real_cnots_benchmark,
        real_depth_benchmark,
            ...
            for strategy in ["brute_force", "greedy"]:
                if not use_parallel:
                    print(
                        ...
                        f"{'With' if use_low_rank else 'No'} Low Rank Processing....",
                        end="",
                    )
                start_time = datetime.datetime.now()
                real_cnots, real_depth, real_fidelity_loss, node = initialize_loss(
                    state_vector=state_vector,
                    fidelity_loss=max_fidelity_loss,
                    use_low_rank=use_low_rank,
                    strategy=strategy,
                )
                ...
                data = [
                    ...
                    mw_entanglement,
                    max_fidelity_loss,
                    node.total_saved_cnots,
                    node.total_fidelity_loss,
                    vector_data,
                    real_cnots,
                    real_cnots_benchmark,
                    real_depth,
...
class TestBaa(TestCase):
    columns = [
        ...
        "real_cnots_benchmark",
        "real_depth",
        "real_depth_no_approx",
        "real_fidelity_loss",
        "real_fidelity_loss_benchmark",
        "duration",
        "predicted_cnots",
        "benchmark_fidelity_loss_pass",
        "approximation_calculation_pass",
    ...
    def test(self):
        ...
        test_data = np.hstack(
            [
                ...
                (
                    ...
                    - test_data[:, self.columns.index("total_saved_cnots")]
                ).reshape(-1, 1),
            ]
        )
        ...
        test_data = np.hstack(
            [
                ...
                (
                    np.abs(
                        test_data[:, self.columns.index("real_fidelity_loss_benchmark")]
                    )
                    < 1e-6
                ).reshape(-1, 1),
            ]
        ...
        benchmark_fidelity_loss_fail_count = (
            test_data.shape[0]
            - test_data[
                test_data[:, self.columns.index("benchmark_fidelity_loss_pass")].astype(
                    bool
                )
            ].shape[0]
        )
        approximation_calculation_fail_count = (
            ...
            - test_data[
                test_data[
                    :, self.columns.index("approximation_calculation_pass")
                ].astype(bool)
            ].shape[0]
        )
        real_approximation_calculation_fail_count = (
            ...
            - test_data[
                test_data[
                    :, self.columns.index("real_approximation_calculation_pass")
                ].astype(bool)
            ].shape[0]
        )
        if benchmark_fidelity_loss_fail_count > 0:
            print(
                f"[WARNING] NOT ALL benchmark_fidelity_loss_pass are true ({benchmark_fidelity_loss_fail_count})",
                file=sys.stderr,
            )
        ...
        if approximation_calculation_fail_count > 0:
            print(
                f"[WARNING] NOT ALL approximation_calculation_pass are true ({approximation_calculation_fail_count})",
                file=sys.stderr,
            )
        ...
        if real_approximation_calculation_fail_count > 0:
            print(
                f"[WARNING] NOT ALL real_approximation_calculation_pass are true "
                f"({real_approximation_calculation_fail_count})",
                file=sys.stderr,
            )
        if real_approximation_calculation_fail_count > fails_to_still_pass:
            print(
                "[FAIL] real_approximation_calculation_pass must be true",
                file=sys.stderr,
            )
            test_passed = False
    ...
    def test_node_state_vector(self):
        ...
        node = baa.adaptive_approximation(
            state, use_low_rank=True, max_fidelity_loss=0.09, strategy="brute_force"
        )
